...
| `POST /api/apply-edits` | Receives `{removals, wpDeleted, wpEdited}`, calls `process_edits_v2.run_edits()` to produce an edited .kmz, returns download URL |
...
### Edit Processing (`process_edits_v2.py`)
...
All HTML and CSS is in `index.html`. The JS (`editor.js`) is an IIFE module with these subsystems:
...
`lib/leaflet.js` and `lib/leaflet.css` — Leaflet 1.x (no package manager).
...
- `editor.js` uses `var` throughout (pre-ES6 style) and avoids any build tooling.
